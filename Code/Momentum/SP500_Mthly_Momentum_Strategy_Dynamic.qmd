...
last_test_date  <- as.Date("2025-11-30")
```
...
```{r}
test_dates <- seq(from = first_test_date, to = last_test_date, by = "month")
...
```{r}
train_end_dates <- floor_date(as.Date(test_dates), "month") - days(1)
...
```{r}
  ...
  mutate(roll_id = as.integer(roll_id))
```
