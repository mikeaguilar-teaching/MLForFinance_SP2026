...
```{r}
SP500_Mthly_Returns <- ?(here("Data","SP500_Mthly_SimpleReturns.csv"))
```
...
```{r}
SP500_Daily_SimpleReturns <- ?(here("Data","SP500_Daily_SimpleReturns.csv"))
```
...
constituent_names <- constituent_names[constituent_names != ?]
```
...
SR_Matrix <- Train.Returns.Constituents %>%
  ...
  group_by(ticker) %>%
  summarize(
    mean_return = ?(OneMthSimpleRet, na.rm = TRUE),
    sd_return = ?(OneMthSimpleRet, na.rm = TRUE)
  ) %>%
  mutate(
    sharpe_ratio = ?
  ) %>%
...
SR_Signal_Matrix <- SR_Matrix %>%
  mutate(
    ? = ifelse(sharpe_ratio > 0, sharpe_ratio, 0),
    Date = train_end_date
  ) %>%
...
Weights.SR <- SR_Signal_Matrix %>%
  mutate(weight = SR_Signal_Matrix$signal_matrix / ?) %>%
...
returns_df <- temp1 %>%
  left_join(temp2, by = "?", suffix = c("_Momentum", "_SR")) %>%
  rename(Date = date,
...
```{r}
mu_s <- ?(returns_matrix, na.rm = TRUE)   # length m
Sigma_s <- ?(returns_matrix, use = "pairwise.complete.obs")  # m x m
```
...
if (normalize_weights) {
  ...
  if (sabs == 0) stop("All implied weights are zero — nothing to construct.")
