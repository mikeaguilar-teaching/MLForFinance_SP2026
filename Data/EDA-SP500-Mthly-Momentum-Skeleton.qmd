...
Avg_Momentum_ByAsset <- Train.Momentum %>%
  ...
  group_by(ticker) %>%
  summarise(avg_momentum = ?(?, na.rm = TRUE), .groups = "drop") %>%
...
Avg_Momentum_Recent_ByAsset <- Train.Momentum %>%
  ...
  group_by(ticker) %>%
  ?(order_by = Date, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
...
```{r}
last_train_date <- Train.Momentum %>% ?(d = max(Date)) %>% pull(d)
first_test_date <- Test.Returns  %>% summarize(d =?(Date)) %>% pull(d)
...
```{r}
nov_cs <- signal %>%
  inner_join(returns, ? = "ticker") %>%   # keep only tickers present in both months
...
```{r}
ggplot(decile_summary, aes(x = factor(decile), y = ?)) +
  geom_col(width = 0.7) +
