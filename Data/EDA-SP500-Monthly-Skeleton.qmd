...
?(here("Supporting", "PackageLoads.R"))
...
SP500_Mthly_SimpleReturns<- ?(here("Data","SP500_Mthly_SimpleReturns.csv"))
```
...
Mean_1m_Returns_Train <- Train.Constituents %>%
  group_by(ticker) %>%
  summarise(mean_ret_1m = ?(OneMthSimpleRet, na.rm = TRUE), .groups = "drop") %>%
...
ret_stats <- tibble(
  ...
  q_90  = unname(quantile(x, ?, na.rm = TRUE)),
...
SD_1m_Returns_ByAsset <- Train.Constituents %>%
  group_by(ticker) %>%
  summarise(sd_ret_1m = ?(OneMthSimpleRet, na.rm = TRUE), .groups = "drop") %>%
...
Skew_1m_Returns_ByAsset <- Train.Constituents %>%
  group_by(ticker) %>%
  summarise(skew_ret_1m = moments::?(OneMthSimpleRet, na.rm = TRUE), .groups = "drop") %>%
...
Kurt_1m_Returns_ByAsset <- Train.Constituents %>%
  group_by(ticker) %>%
  summarise(kurt_ret_1m = moments::?(OneMthSimpleRet, na.rm = TRUE), .groups = "drop") %>%
...
```{r}
correlation <- ?(df$train_mean_ret, df$test_ret, use = "complete.obs")
correlation
```
...
test_ranked <- test %>%
  mutate(
    test_rank = ?(test_ret, na.last = "keep", ties.method = "average")
   ) %>%
  ungroup() %>%
  arrange((test_ret))
...
rank_corr <- ?(rank_df$train_rank, rank_df$test_rank, use = "complete.obs", method = "pearson")
rank_corr
```
