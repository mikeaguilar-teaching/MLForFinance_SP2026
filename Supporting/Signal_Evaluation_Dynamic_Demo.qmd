...
n_assets <- 200
tickers <- paste0("T", sprintf("%03d", 1:n_assets))
```
...
panel <- panel %>%
  mutate(
    test_returns = 0.003 * score + rnorm(n(), 0, 0.02)
  )
```
...
```{r}
res_dyn <- signal_evaluation_dynamic(
